...
const PromptCardComponent: React.FC<PromptCardProps> = ({
  ...
  return (
    ...
    >
      <div className={`relative z-10 flex h-full flex-col ${FOUNDATION.spacing['xl']} ${LAYOUT.spacing.component}`}>
        {/* Header Section - 统一间距 */}
        <div className={`flex items-start justify-between ${FOUNDATION.spacing['lg']}`}>
        ...
        <div className={`${FOUNDATION.borderRadius.lg} border ${tone.preview} bg-[var(--color-bg-secondary)]`}>
          <div className={`flex items-center justify-between ${FOUNDATION.spacing['lg']} ${FOUNDATION.spacing['lg']}`}>
            <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide">
              <Icons.Sparkles size={12} />
              <span>Preview</span>
            </div>
          </div>
          <div className={`${FOUNDATION.spacing['lg']} pb-3 h-[80px] overflow-hidden`}>
            <PromptContentPreview content={previewSnippet} variant={themeVariant} />
          </div>
        </div>

        {/* Stats Row */}
        <div className={`flex flex-nowrap items-center ${FOUNDATION.spacing['lg']} text-xs text-slate-400/90`}>
          <span className="inline-flex items-center gap-1 whitespace-nowrap">
            <Icons.Activity size={10} className="text-slate-500" /> ~{tokens} tks
          </span>
          <span className="inline-flex items-center gap-1 whitespace-nowrap">
            <Icons.Eye size={10} className="text-slate-500" /> {usageCount} uses
          </span>
          <span className="inline-flex items-center gap-1 whitespace-nowrap">
            <Icons.History size={10} className="text-slate-500" /> {updatedDate}
          </span>
        </div>
