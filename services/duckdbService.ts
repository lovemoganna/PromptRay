  ...
  try {
    ...
    // 处理数据修改操作
      ...
      if (sqlLower.includes('into prompts')) {
        return [{ affected_rows: 1, message: 'Record inserted successfully' }];
      }
    }

    else if (sqlLower.startsWith('update')) {
      if (sqlLower.includes('prompts set')) {
        return [{ affected_rows: 1, message: 'Record updated successfully' }];
      }
    }

    else if (sqlLower.startsWith('delete')) {
      if (sqlLower.includes('from prompts')) {
        return [{ affected_rows: 1, message: 'Record deleted successfully' }];
      }
    }
